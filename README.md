...
The diagram below shows the complete lifecycle of a compliance scan — from trigger to remediation to audit evidence.
...
- **ACCESS_VIOLATION** — terminated employee still has active access
- **INACTIVE_ADMIN** — admin account dormant beyond 90-day threshold
- **SHARED_ACCOUNT** — generic/shared account with elevated privileges
- **PERMISSION_CREEP** — user's role should never have admin access
...
### Phase 5 — Remediation Execution (Nova Act)

When a remediation is approved, the **Remediation Engine** launches a new Nova Act session targeting the specific legacy tool. It logs in, navigates to the user record, and executes the appropriate action (disable account, revoke admin, downgrade role). A confirmation screenshot is captured as proof. Shared account violations are flagged for manual review only — no autonomous action is taken.

### Phase 6 — Audit & Reporting

Every action (scan started, scan completed, violation detected, remediation executed, violation dismissed) is logged to the **audit trail** with timestamps, actor names, and screenshot paths. The engineer can export a professional **PDF audit report** generated by Nova 2 Lite, suitable for SOC2 auditor review.
